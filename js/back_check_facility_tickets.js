...
for (var i = 0; i < facility_icon.length; i++) {
    ...
    facility_icon[i].addEventListener('click', function () {//抓到data-title裡面的值
        ...
        show_where_photo.setAttribute('src', '/future-atlas/img/secondSection/' + now_facility_english + '_img.jpg');
    ...
    facility_icon[i].addEventListener('click', setLocalStorage);  //寫入localStorage

    function setLocalStorage() {
        alert(now_facility_chinese);
        localStorage.setItem('now_facility_chinese',2);
        localStorage.setItem('now_facility_english',3);
...
//             full_num.value = xhr.responseText;
//         }
//         else
//         {
//             alert(xhr.status);
...
//     var url = "back_check_facility_tickets.php";
